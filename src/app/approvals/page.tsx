...
export default function ApprovalsPage() {
  ...
  const handleVerify = async (appId: string) => {
    ...
    try {
      ...
      setAiSummaries((prev) => ({
        ...prev,
        [appId]: { summary: "Failed to verify documents." },
      }));
    } finally {
    ...
    try {
      ...
      setActionStatuses((prev) => ({
        ...
        [id]: {
          itemId: id,
          action: "rejected",
          message: `❌ Rejected: Reimbursement request for ${name} has been rejected and returned to the employee for correction.`,
        },
      }));
    } catch (e) {
      console.error(e);
      ...
      >
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground mt-1">
            ...
            reports and department employees.
          </p>
        </div>
          ...
          {approvals.length === 0 ? (
            ...
            approvals.map((approval, idx) => {
              ...
              const employee = mockEmployees.find(
                (e) => e.id === approval.employeeId,
              );
              const displayEmpId = employee
                ? employee.employeeId
                : approval.employeeId;
                  ...
                  <CardContent className="p-0 flex flex-col">
                    ...
                    <div className="py-3 px-4 flex flex-col md:flex-row md:items-start justify-between gap-3">
                      <div className="flex-1">
                        ...
                        {isSuspended && (
                          <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-none flex items-start gap-2 text-amber-600">
                        ...
                        {actionResult && (
                          <div
                            className={`mt-4 p-3 rounded-none border flex items-start gap-2 text-xs font-medium leading-relaxed ${
                              actionResult.action === "approved"
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-3 pt-3 border-t border-border bg-muted/10 space-y-3 animate-in fade-in duration-200">
                            ...
                            <div className="py-2.5 space-y-2">
                              ...
                              <div className="space-y-2">
                                {approval.documents.map((doc: any) => (
                                  <div
                                    key={doc.id}
                                    className="flex items-center justify-between p-2 rounded-none border border-border bg-card"
                                  >
                        ...
                        {/* AI Summary details */}
                        <div className="py-2.5 px-3 rounded-none bg-primary/5 border border-primary/10 space-y-3">
                          ...
                          {aiSummaries[approval.id] &&
                          aiSummaries[approval.id].studentName !== undefined ? (
                              ...
                              <div className="grid grid-cols-2 gap-2 text-xs">
                                <div>
                                  ...
                                  <p className="font-semibold text-foreground mt-0.5">
                                    {aiSummaries[approval.id].studentName ||
                                      "N/A"}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p className="font-semibold text-foreground mt-0.5">
                                    {aiSummaries[approval.id].institution ||
                                      "N/A"}
                                  </p>
                                </div>
                                <div>
                                  ...
                                  <p className="font-semibold text-foreground mt-0.5">
                                    {aiSummaries[approval.id].term || "N/A"}
                                  </p>
                                </div>
                                <div>
                                  <span className="text-[9px] font-bold text-muted-foreground uppercase">
                                  ...
                                  <p className="font-semibold text-foreground mt-0.5">
                                    {aiSummaries[approval.id].fees || "N/A"}
                                  </p>
                                </div>
                              </div>
                              {aiSummaries[approval.id].courses &&
                                aiSummaries[approval.id].courses.length > 0 && (
                                  <div>
                                    <span className="text-[9px] font-bold text-muted-foreground uppercase">
                                      Courses Detected
                                    </span>
                                    <div className="flex flex-wrap gap-1 mt-1">
                                      {aiSummaries[approval.id].courses.map(
                                        (c: string, i: number) => (
                                          <Badge
                                            key={i}
                                            variant="secondary"
                                            className="text-[10px] font-semibold bg-primary/10 text-primary hover:bg-primary/20 border-transparent"
                                          >
                                            {c}
                                          </Badge>
                                        ),
                            ...
                            <p className="text-xs text-muted-foreground leading-relaxed italic border-t border-primary/10 pt-2">
                              {aiSummaries[approval.id]?.summary ||
                                (typeof aiSummaries[approval.id] === "string"
                                  ? aiSummaries[approval.id]
                                  : "Click verify to generate AI summary.")}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-col md:flex-row items-start md:items-center gap-2 p-2.5 rounded-none border border-border bg-card text-xs">
                            <div className="flex items-center gap-1.5">
                          ...
                          <div className="relative pl-4 space-y-3.5 border-l border-border/80 ml-2 pt-2">
                            <div className="relative">
                              ...
                              <p className="text-[9px] text-muted-foreground mt-0.5">
                                Submitted by employee {approval.employeeName} •{" "}
                                {formatDate(approval.submittedDate)}
                              </p>
                            </div>
                            <div className="relative">
                            ...
                            <div className="relative">
                              ...
                              <p className="text-[9px] text-muted-foreground mt-0.5">
                                Routed to designated Nurse Manager Maria Santos
                                • {formatDate(approval.submittedDate)}
                              </p>
                            </div>
                            {actionResult && (
