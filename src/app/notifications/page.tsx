...
export default function NotificationCenter() {
  ...
  const selectedNotifData =
    notifications.find((n) => String(n.id) === selectedNotifId) ||
    filteredNotifs[0];
  const activeThread =
  ...
  return (
    ...
    >
      <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto bg-white min-h-full text-left">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-normal font-display text-[#003769]">
              Application Status & Communication Hub
            </h1>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button
              variant="outline"
              size="sm"
              className="h-9 w-9 p-0 text-[#003769] hover:bg-[#E6F0F5] rounded-full border-slate-300"
            >
        ...
        <div className="flex border-b border-slate-200 gap-6">
          <button
            ...
            className={`py-2 px-1 text-xs font-bold border-b-2 rounded-none transition-all cursor-pointer ${
              activeTab === "status"
                ? "border-[#008573] text-[#008573]"
                : "border-transparent text-slate-500 hover:text-[#003769]"
            }`}
          >
            Application Status Updates ({notifications.length})
          </button>
          <button
            type="button"
            onClick={() => {
          ...
          >
            Official Communication Threads ({threads.length})
          </button>
        </div>
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        ...
                        if (!notif.read)
                          markAsRead.mutateAsync(String(notif.id));
                      }}
                    >
                    ...
                    <CardHeader className="pb-3 border-b border-border flex flex-row justify-between items-start gap-4">
                      <div className="space-y-1.5">
                        ...
                        <p className="text-[10px] text-slate-500">
                          Logged:{" "}
                          {new Date(
                            selectedNotifData.createdAt,
                          ).toLocaleString()}
                        </p>
                      </div>
                    </CardHeader>
                    <CardContent className="p-6 space-y-6">
                      <div className="p-5 rounded-none border border-[#acd3c0] bg-[#ebf3ef]/40">
                        <p className="text-xs text-[#1A1A1A] leading-relaxed font-body">
                      ...
                      <div className="space-y-2">
                        <h4 className="text-[10px] font-bold text-[#008573] uppercase tracking-wider">
                  ...
                  {selectedNotifData.link && (
                    <div className="p-4 bg-white border-t border-slate-200 flex justify-end">
                      <Link href={selectedNotifData.link} passHref>
                        <Button className="h-9 gap-1.5 text-xs font-bold rounded-full bg-[#003769] text-white hover:bg-[#00274d] border-0 shadow-xs px-5">
