          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
            <div>
              <h1 className="text-2xl font-bold font-display text-foreground">
                Service Agreement Tracker
              </h1>
              <p className="text-xs text-muted-foreground mt-0.5">
                Monitoring 12-month post-reimbursement employment commitments
              </p>
            </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/40 hover:bg-muted/40 border-b border-border">
                      <TableHead className="text-[10px] font-bold uppercase px-6 h-11">
                        Employee
                      </TableHead>
                      <TableHead className="text-[10px] font-bold uppercase h-11">
                        Amount
                      </TableHead>
                      <TableHead className="text-[10px] font-bold uppercase h-11">
                        Commitment End
                      ...
                      <TableHead className="text-[10px] font-bold uppercase h-11 text-right pr-6">
                        Actions
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                      ...
                      >
                        ...
                        <TableCell>
                          <div className="flex flex-col">
                            ...
                            <span
                              className={`text-[9px] font-bold ${calculateDaysRemaining(sa.endDate) < 30 ? "text-destructive" : calculateDaysRemaining(sa.endDate) < 90 ? "text-amber-600" : "text-muted-foreground"}`}
                            >
                              {calculateDaysRemaining(sa.endDate)} days
                              remaining
                            </span>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={`text-[9px] h-5 px-1.5 ${getStatusColor(sa.status)}`}
                          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 shrink-0">
                ...
                <CardContent className="p-6 space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-bold text-foreground">
                      {formatCurrency(selectedSa.amount)}
                    </span>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-[10px] font-bold text-muted-foreground uppercase">
                      ...
                      <span>
                        {100 -
                          Math.round(
                            (calculateDaysRemaining(selectedSa.endDate) / 365) *
                              100,
                          )}
                        %
                      </span>
                    ...
                    <Progress
                      value={
                        100 -
                        (calculateDaysRemaining(selectedSa.endDate) / 365) * 100
                      }
                  ...
                  <div className="p-4 rounded-xl bg-destructive/5 border border-destructive/10 space-y-3">
                    ...
                    <p className="text-[11px] text-destructive/80 leading-relaxed">
                      If the employee departs today, the required repayment
                      amount under the signed agreement is:
                    </p>
                    <div className="text-3xl font-bold text-destructive">
                      {formatCurrency(selectedSa.proratedRepayment)}
                    </div>
              ...
              <Card className="border-border shadow-sm bg-card">
                ...
                <CardContent className="p-6 space-y-6">
                  ...
                  <div className="pt-2">
                    <Button
                      className="w-full bg-primary text-white font-bold h-10 gap-2 shadow-sm"
                      onClick={() =>
                        window.open(selectedSa.docusignUrl, "_blank")
                      }
                    >
                    ...
                    <Button
                      variant="ghost"
                      className="w-full mt-2 h-9 text-xs text-muted-foreground gap-2"
                    >
            ...
            <Card className="border-amber-500/20 bg-amber-500/5 overflow-hidden shadow-sm">
              <CardHeader className="p-4 border-b border-amber-500/10">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-xs font-bold uppercase tracking-widest text-amber-700">
                    Expiration Alerts
                  </CardTitle>
                </div>
              </CardHeader>
              <CardContent className="p-4 space-y-4">
                {[
                  ...
                  <div
                    key={i}
                    className="bg-white p-3 rounded-lg border border-amber-200 shadow-sm space-y-2"
                  >
                    <div className="flex justify-between items-start">
                      <p className="text-xs font-bold">{alert.name}</p>
                      <Badge className="bg-amber-100 text-amber-700 text-[8px] h-4 border-transparent">
                        T-{alert.days}d
                      </Badge>
                    </div>
                    <p className="text-[10px] text-muted-foreground leading-tight">
                      Agreement commitment ends in {alert.days} days. Ensure
                      employee remains in good standing.
                    </p>
                  </div>
                ))}
              </CardContent>
