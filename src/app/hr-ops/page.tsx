        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              NYSNA compliance | Case queue | SLA monitoring | AI Insights
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" className="h-9 gap-2 bg-primary shadow-sm">
            ...
            <Card key={i} className="border-border shadow-sm">
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div
                    className={`p-1.5 rounded-lg ${stat.alert ? "bg-destructive/10 text-destructive" : stat.success ? "bg-emerald-500/10 text-emerald-500" : "bg-muted text-muted-foreground"}`}
                  >
                ...
                <div className="flex items-baseline gap-2 mb-1">
                  ...
                  {stat.badge && (
                    <Badge
                      variant="destructive"
                      className="text-[9px] h-4 px-1 font-bold bg-destructive/10 text-destructive border-transparent"
                    >
                      {stat.badge}
                    </Badge>
                  )}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-border shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-bold">
                  NYSNA Compliance Tracker
                </CardTitle>
                <p className="text-[10px] text-muted-foreground">
                  Article 35 - 45-Day Processing Requirement
                </p>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div
                      key={i}
                      className={`flex items-center justify-between p-3 rounded-xl border ${c.alert ? "bg-destructive/5 border-destructive/20" : "bg-amber-500/5 border-amber-500/20 shadow-sm"}`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center ${c.alert ? "bg-destructive/10 text-destructive" : "bg-amber-500/10 text-amber-500"}`}
                        >
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 px-1">
                ...
                <Badge className="bg-destructive/10 text-destructive text-[9px] h-4 font-bold border-transparent">
                  2 Active
                </Badge>
              </div>
                ...
                <Card className="border-border shadow-sm hover:shadow-md transition-shadow">
                  <CardContent className="p-4 space-y-4">
                    ...
                    <div className="bg-destructive/5 text-[10px] text-destructive p-3 rounded-lg border border-destructive/10 font-medium leading-relaxed">
                      Service agreement not completed - NYSNA 45-day window
                      exceeded
                    </div>
                    <div className="flex gap-1.5 justify-end">
                      ...
                      <Button
                        size="sm"
                        className="h-7 text-[10px] px-3 font-bold bg-destructive text-white"
                      >
                        Resolve
                      </Button>
                    </div>
                ...
                <Card className="border-border shadow-sm hover:shadow-md transition-shadow">
                  <CardContent className="p-4 space-y-4">
                    ...
                    <div className="bg-amber-500/5 text-[10px] text-amber-700 p-3 rounded-lg border border-amber-500/10 font-medium leading-relaxed">
                      Low AI confidence (71%) - Invoice requires manual document
                      review
                    </div>
                    <div className="flex gap-1.5 justify-end">
                      <Button
                      ...
                      <Button
                        size="sm"
                        className="h-7 text-[10px] px-3 font-bold bg-amber-500 text-white"
                      >
                        Resolve
                      </Button>
                    </div>
                ...
                <div className="px-6 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <Tabs defaultValue="all" className="w-auto">
                    <TabsList className="bg-brand-lightblue p-1 h-auto rounded-lg gap-1 border-none">
                      <TabsTrigger
                        value="all"
                        className="text-[10px] font-bold px-3 py-1.5 rounded-md data-[state=active]:bg-white data-[state=active]:shadow-sm"
                      >
                        All Cases{" "}
                        <span className="ml-1 px-1.5 rounded-full bg-muted text-muted-foreground">
                          10
                        </span>
                      </TabsTrigger>
                      <TabsTrigger
                      ...
                      >
                        High Priority{" "}
                        <span className="ml-1 px-1.5 rounded-full bg-destructive/10 text-destructive">
                          4
                        </span>
                      </TabsTrigger>
                      <TabsTrigger
                      ...
                      >
                        Pending Review{" "}
                        <span className="ml-1 px-1.5 rounded-full bg-amber-100 text-amber-700">
                          3
                        </span>
                      </TabsTrigger>
                    </TabsList>
                ...
                <div className="px-6 py-4 bg-muted/10 border-t border-border flex flex-wrap items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Select defaultValue="all">
                      <SelectTrigger className="h-10 text-xs w-[130px] bg-background shadow-sm">
              ...
              <CardContent className="p-0">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="hover:bg-transparent bg-muted/40 border-b border-border">
                        ...
                        <TableHead className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground h-11 text-right pr-6">
                          Actions
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredCases.map((c, idx) => (
                        <TableRow
                          key={c.id}
                          className="hover:bg-muted/30 transition-all border-b border-border/40 group cursor-pointer"
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge
                              variant="outline"
                              className="text-[9px] font-bold bg-muted/30 h-5 px-2 border-transparent"
                            >
                              {c.programType === "TuitionReimbursement"
                                ? "Tuition"
                                : "Other"}
                            </Badge>
                          </TableCell>
                          ...
                          <TableCell className="text-right pr-6">
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-8 text-[10px] font-bold px-3 hover:bg-primary/10 hover:text-primary transition-all"
                            >
                              Review
                            </Button>
                          </TableCell>
                ...
                <div className="px-6 py-4 border-t border-border bg-muted/5 flex items-center justify-between">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                    Showing 10 of 124 cases
                  </span>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-8 text-[10px] font-bold px-4 shadow-sm"
                    >
                      Prev
                    </Button>
                    <Button
            ...
            <Card className="border-border shadow-sm overflow-hidden">
              ...
              <CardContent className="p-4 space-y-4">
                {[
                  {
                    id: 1,
                    title: "3 applications approaching SLA",
                    text: "Cases 009, 010, and 002 will breach soon.",
                    color: "border-amber-200 bg-amber-50/50",
                  },
                  {
                    id: 2,
                    title: "Unusual spike in CME",
                    text: "CME submissions are up 42%.",
                    color: "border-blue-200 bg-blue-50/50",
                  },
                ].map((rec) => (
                  <div
                    key={rec.id}
                    className={`p-4 rounded-xl border ${rec.color} space-y-2 shadow-sm`}
                  >
            ...
            <Card className="border-border shadow-sm overflow-hidden h-fit sticky top-6">
              <CardHeader className="py-3 border-b border-border bg-muted/30">
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    variant="outline"
                    className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[9px] h-5 gap-1.5 px-2 font-bold shadow-sm"
                  >
              ...
              <CardContent className="p-4 space-y-5">
                ...
                <Button
                  variant="ghost"
                  className="w-full h-10 text-xs font-bold text-muted-foreground border-t border-border rounded-none -mx-4 -mb-4 mt-4"
                >
                  View All Activity
                </Button>
              </CardContent>
