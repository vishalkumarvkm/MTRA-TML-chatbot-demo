        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold font-display text-foreground">
              Workday Payroll Feed
            </h1>
            <p className="text-xs text-muted-foreground mt-0.5">
              Automated reimbursement posting | Tax-split calculation | GL
              mapping
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="h-9 gap-2 shadow-sm">
              <RefreshCw className="w-3.5 h-3.5" /> Sync Workday
            </Button>
            <Button size="sm" className="h-9 gap-2 bg-primary shadow-sm">
              <Download className="w-3.5 h-3.5" /> YTD Tax Report
            </Button>
          </div>
        </div>
            ...
            <Card className="border-border shadow-sm overflow-hidden">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="h-8 pl-8 pr-4 text-[10px] rounded-lg border border-border bg-background w-[180px]"
                      />
                    </div>
                    <Button
                      variant="outline"
              ...
              <CardContent className="p-0">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-muted/10">
                        <TableHead className="text-[10px] font-bold uppercase px-6 h-10 w-[50px]">
                    ...
                    <TableBody>
                      {POSTINGS.map((p) => (
                        <TableRow
                          key={p.id}
                          className="hover:bg-muted/20 border-b border-border/50 transition-colors"
                        >
                          ...
                          <TableCell>
                            <Badge
                              variant="outline"
                              className={`text-[8px] h-4 px-1 ${p.tax.includes("Taxable") ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-blue-50 text-blue-700 border-blue-200"}`}
                            >
                              {p.tax}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-right pr-6">
                            <Button
                              size="sm"
                              variant={
                                reconciled.includes(p.id) ? "ghost" : "outline"
                              }
                ...
                <div className="p-4 bg-muted/5 flex justify-end gap-2 border-t border-border">
                  ...
                  <Button
                    size="sm"
                    className="h-9 px-6 bg-primary font-bold text-xs"
                  >
                    Post to Workday (3 Selected)
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-violet-500/20 bg-violet-500/5 shadow-sm">
              ...
              <CardContent className="p-4 space-y-4">
                <p className="text-[10px] text-muted-foreground leading-relaxed">
                  High-value Dependent Tuition cases (Program 4) require manual
                  withholding verification for correct W-2 reporting.
                </p>
                <div className="bg-white p-3 rounded-lg border border-violet-200 shadow-sm space-y-3">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full h-7 text-[9px] font-bold border-violet-200 text-violet-700 hover:bg-violet-50"
                  >
                    Verify Calculation
                  </Button>
                </div>
              ...
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[9px] text-emerald-700/80">
                    TLS 1.3 encrypted tunnel to Workday REST API active.
                  </p>
                </div>
              </CardContent>
            </Card>
