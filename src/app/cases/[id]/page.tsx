...
const CHECKLIST_ITEMS = [
  { id: "submissionPath", label: "ServiceNow Path Valid", desc: "Submitted through 'Apply for Tuition Reimbursement or Scholarship Programs'" },
  { id: "correctProgram", label: "Correct Program Type", desc: "Matches programType to required documents" },
  { id: "completedApplication", label: "Application Completed", desc: "No blank required fields (EZID, semester, campus, etc.)" },
  { id: "allSignatures", label: "Required Signatures Present", desc: "Manager/employee signatures validated" },
  { id: "billQuality", label: "Itemized School Bill Attached", desc: "Excludes late fees, payment plans, books" },
  { id: "gradesQuality", label: "Grades Verified (C or better)", desc: "Student name visible, no 'Enrolled' or 'In Progress' states" },
  { id: "serviceAgreement", label: "Service Agreement Notarized", desc: "Required if NYSNA RN or amount > $3,000" },
  { id: "cmeSignatures", label: "CME Specific Approvers Met", desc: "PT (Asst Chief) or PA (Dept Admin) signoff" },
  { id: "acceptanceLetter", label: "Freshman Acceptance Letter", desc: "Children's Tuition program only" },
  { id: "scholarshipDeadline", label: "Scholarship Deadline Met", desc: "Submitted by July 1 with school seal + FAO signature" },
  { id: "documentQuality", label: "Documents Legible PDF", desc: "All docs in single, clear PDF; no hyperlinks" },
  { id: "noDuplicates", label: "No Duplicate Applications", desc: "No duplicate open tickets for same semester/course" },
  { id: "notCertification", label: "Not Certification Course", desc: "Excluded certification types (ACLS, BLS, PALS)" },
  { id: "withinTimeWindow", label: "Submission Within Window", desc: "45 days from course start / 90 days from course completion" }
];
...
export default function CaseDetailsPage({
  ...
  const [tier1Status, setTier1Status] = useState<'approved' | 'incomplete' | 'flagged' | 'escalated'>(() => {
  ...
  const [hwStatus, setHwStatus] = useState<'pending' | 'under-review' | 'pending-info' | 'approved' | 'rejected'>(() => {
  ...
  const [payrollStatus, setPayrollStatus] = useState<'awaiting' | 'processing' | 'processed'>(() => {
    ...
    return [
      {
        ...
        description: "Application submitted for Tuition Reimbursement via ServiceNow portal."
      },
      {
        id: "a-2",
        ...
        entityId: caseData.applicationId,
        entityType: "system",
        description: "AI extraction completed. Confidence score: 94%. Checked department rules."
      },
      {
        id: "a-3",
  ...
  useEffect(() => {
    ...
    const allIntakeChecked = CHECKLIST_ITEMS.every(item => {
      ...
      if (item.id === "cmeSignatures" && caseData.programType !== "CMEReimbursement") return true;
      if (item.id === "acceptanceLetter" && caseData.programType !== "DependentTuition") return true;
      if (item.id === "scholarshipDeadline" && caseData.programType !== "MMCScholarship") return true;
      if ((item.id === "billQuality" || item.id === "gradesQuality") && !isTuition) return true;
    ...
    if (allIntakeChecked && tier1Status === 'incomplete') {
      ...
      logAction("TIER1_COMPLIANCE_PASSED", "System", "admin", "All required 14 checklist items checked and passed.");
    } else if (!allIntakeChecked && tier1Status === 'approved') {
      setTier1Status('incomplete');
      logAction("TIER1_COMPLIANCE_FAILED", "System", "admin", "Checklist requirements modified. Quality Gate incomplete.");
    }
  }, [checklist]);

  const logAction = (action: string, actor: string, role: UserRole, details: string) => {
  ...
  const handleContextualApprove = () => {
    if (currentStage === 1) {
      if (tier1Status !== 'approved') {
        alert("Cannot approve Stage 1. Please ensure all required checklist items are checked in the 'SOP Checklist' tab.");
        return;
      }
      setCurrentStage(2);
      ...
      if (signatures.admin.status !== 'complete' || signatures.deptHead.status !== 'complete') {
        alert("Pending signatures. Complete all required signatures in the 'Timeline' stage below to proceed.");
        return;
      }
      // Check notarized SA if required
      const needsSA = caseData.programType === "TuitionReimbursement" && (employee.isNYSNA || caseData.amount > 3000);
      if (needsSA && !checklist.serviceAgreement) {
        alert("Blocking Rule: Notarized Service Agreement required for payroll posting.");
        return;
      }
      setCurrentStage(4);
      setPayrollStatus('awaiting');
      logAction("STAGE3_COMPLETED", "System", "admin", "All signatures completed. Routed to Stage 4 Payroll.");
      ...
      logAction("STAGE5_COMPLETED", "System", "admin", "Reimbursement paid in paycheck distribution. Application completed.");
  ...
  const handleAdminSign = () => {
    if (!adminNameInput.trim() || !adminSigChecked || !adminDateInput.trim()) {
      alert("Signature Validation Failed: Printed Name, Checked Signature box, and Date are all required per SOP Section 5.");
      return;
    }
  ...
  const handleDeptHeadSign = () => {
    ...
    if (signatures.admin.status !== 'complete') {
      alert("SOP Blocking Rule: First signature line (Administrator) must complete before Department Head can sign.");
      return;
    }

    if (!deptHeadNameInput.trim() || !deptHeadSigChecked || !deptHeadDateInput.trim()) {
      alert("Signature Validation Failed: Printed Name, Checked Signature box, and Date are all required per SOP Section 5.");
      return;
    }
    const dateRegex = /^\d{2}\/\d{2}\/\d{4}$/;
    ...
    logAction("SIGNATURE_ADDED", "Dr. James Okonkwo", "manager", `Department Head signature verified. Name: ${deptHeadNameInput}. Date: ${deptHeadDateInput}.`);
  ...
  const isChecklistItemRequired = (itemId: string) => {
    if (itemId === "serviceAgreement") {
      return caseData.programType === "TuitionReimbursement" && (employee.isNYSNA || caseData.amount > 3000);
    }
  ...
  const activeAlerts = () => {
    ...
    if (currentStage === 1 && tier1Status === 'incomplete') {
      list.push({
        ...
        message: "All 14 checklist items must be validated in the 'SOP Checklist' tab to proceed."
      });
    }
    if (currentStage === 3 && (signatures.admin.status === 'pending' || signatures.deptHead.status === 'pending')) {
      list.push({
        id: "alert-2",
        severity: "critical" as const,
    ...
    if (caseData.programType === "TuitionReimbursement" && (employee.isNYSNA || caseData.amount > 3000)) {
      if (!checklist.serviceAgreement) {
        list.push({
          id: "alert-3",
          severity: "warning" as const,
          title: "Service Agreement Required",
          message: "A notarized Service Agreement is required per SOP Section 7 for tuition reimbursement. Lock this item before Stage 4."
        });
      }
    }
  ...
  const getStageBadge = (stageNum: number, statusStr: string) => {
    if (currentStage > stageNum || statusStr === 'approved' || statusStr === 'complete' || statusStr === 'processed' || statusStr === 'paid') {
      return <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 text-[9px] uppercase font-bold px-1.5 h-4.5">Verified</Badge>;
    }
    if (currentStage === stageNum) {
      return <Badge className="bg-amber-500/10 text-amber-600 border-amber-500/20 text-[9px] uppercase font-bold px-1.5 h-4.5 animate-pulse">Current</Badge>;
    }
    return <Badge className="bg-slate-100 text-slate-400 border-slate-200 text-[9px] uppercase font-bold px-1.5 h-4.5">Pending</Badge>;
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col xl:flex-row xl:h-full xl:overflow-hidden bg-muted/20">
          ...
          <main className="flex-1 p-6 space-y-6 xl:overflow-y-auto">
             ...
             {activeAlerts().map((alert) => (
               <div 
                 ...
                 className={[
                   ...
                   alert.severity === "critical" && "bg-red-50 border-red-100 text-red-900 dark:bg-red-950/20 dark:text-red-300 dark:border-red-950",
                   alert.severity === "warning" && "bg-amber-50 border-amber-100 text-amber-900 dark:bg-amber-950/20 dark:text-amber-300 dark:border-amber-950",
                   alert.severity === "info" && "bg-blue-50 border-blue-100 text-blue-900 dark:bg-blue-950/20 dark:text-blue-300 dark:border-blue-950",
                 ].join(" ")}
               >
                 {alert.severity === "critical" && <ShieldAlert className="w-4.5 h-4.5 shrink-0 mt-0.5 text-red-600 dark:text-red-400" />}
                 {alert.severity === "warning" && <AlertTriangle className="w-4.5 h-4.5 shrink-0 mt-0.5 text-amber-600 dark:text-amber-400" />}
                 {alert.severity === "info" && <Info className="w-4.5 h-4.5 shrink-0 mt-0.5 text-blue-600 dark:text-blue-400" />}
                 <div className="text-xs">
                    <p className="font-bold">{alert.title}</p>
                    <p className="mt-0.5 text-muted-foreground leading-relaxed">{alert.message}</p>
                 </div>
               </div>
             ))}
             ...
             <Tabs defaultValue="timeline" className="w-full">
                <TabsList className="bg-brand-lightblue p-1.5 h-auto rounded-xl inline-flex items-center gap-1 border-none mb-6 max-w-full overflow-x-auto no-scrollbar">
                   ...
                   >
                      SOP Checklist <span className="bg-brand-blue text-white text-[9px] w-5 h-5 rounded-full flex items-center justify-center">14</span>
                   </TabsTrigger>
                   <TabsTrigger 
                      value="documents" 
                      className="rounded-lg px-4 py-2 text-xs font-bold data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-primary transition-all gap-2"
                ...
                <TabsContent value="timeline" className="pt-0">
                   <Card className="border-border shadow-sm">
                      <CardHeader className="pb-2">
                         <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Authoritative 5-Stage Approval Engine</CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-8 pt-4 relative pb-10">
                         ...
                         <div className="flex gap-6 relative z-10">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border ${currentStage > 1 ? "bg-emerald-500 text-white border-emerald-500" : currentStage === 1 ? "bg-white text-primary border-primary" : "bg-muted text-muted-foreground border-border"}`}>
                               {currentStage > 1 ? <CheckCircle2 className="w-4 h-4" /> : <span className="text-xs">1</span>}
                            </div>
                            <div className="flex-1">
                               <div className="flex items-center gap-2 mb-1 justify-between">
                                  <h3 className="text-xs font-bold text-foreground">Stage 1: Tier 1 Intake & Completeness Review</h3>
                                  {getStageBadge(1, tier1Status)}
                               </div>
                               <p className="text-[11px] text-muted-foreground leading-relaxed">
                                  Enforces SOP Section 10 checks. System validates correct submission path, program rules, document attachments, and signature completeness.
                               </p>
                               {currentStage === 1 && (
                                  <div className="mt-3 p-3 bg-muted/20 border rounded-xl flex items-center justify-between">
                                     <span className="text-[10px] font-bold uppercase text-muted-foreground">Checklist Status: {tier1Status === "approved" ? "Passed ✅" : "Incomplete ⏳"}</span>
                                     <Button 
                                       size="sm" 
                         ...
                         {/* STAGE 2 */}
                         <div className="flex gap-6 relative z-10">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border ${currentStage > 2 ? "bg-emerald-500 text-white border-emerald-500" : currentStage === 2 ? "bg-white text-primary border-primary" : "bg-muted text-muted-foreground border-border"}`}>
                               {currentStage > 2 ? <CheckCircle2 className="w-4 h-4" /> : <span className="text-xs">2</span>}
                            </div>
                            <div className="flex-1">
                               <div className="flex items-center gap-2 mb-1 justify-between">
                                  <h3 className="text-xs font-bold text-foreground">Stage 2: Health & Wellbeing Review</h3>
                               ...
                               {currentStage === 2 && (
                                  <div className="mt-3 p-3 bg-muted/20 border rounded-xl flex items-center justify-between">
                                     <span className="text-[10px] font-bold uppercase text-muted-foreground">H&W Status: {hwStatus}</span>
                                     <Button 
                                       size="sm" 
                                       onClick={() => {
                                         setHwStatus('approved');
                         ...
                         {/* STAGE 3 */}
                         <div className="flex gap-6 relative z-10">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border ${currentStage > 3 ? "bg-emerald-500 text-white border-emerald-500" : currentStage === 3 ? "bg-white text-primary border-primary" : "bg-muted text-muted-foreground border-border"}`}>
                               {currentStage > 3 ? <CheckCircle2 className="w-4 h-4" /> : <span className="text-xs">3</span>}
                            </div>
                            <div className="flex-1">
                               <div className="flex items-center gap-2 mb-1 justify-between">
                                  <h3 className="text-xs font-bold text-foreground">Stage 3: Approval Signatures (3-Part Validation)</h3>
                                  {getStageBadge(3, signatures.deptHead.status)}
                               </div>
                               <p className="text-[11px] text-muted-foreground leading-relaxed">
                                  Validates sequential approval signoff per program rules. A signature is ONLY valid if it has printed name + handwritten signature + date.
                               </p>
                               
                               {/* Signatures List */}
                               <div className="mt-3 space-y-2">
                                  <div className="flex items-center justify-between text-[11px] p-2 bg-muted/20 rounded-lg border">
                                     <span>Line 1: <strong>Employee</strong> ({employee.name})</span>
                                     <Badge className="bg-emerald-50 text-emerald-700 text-[9px]">✅ Signed {signatures.employee.date}</Badge>
                                  </div>
                                  ...
                                  <div className="flex items-center justify-between text-[11px] p-2 bg-muted/20 rounded-lg border">
                                     ...
                                     {signatures.admin.status === "complete" ? (
                                        ...
                                        <Badge className="bg-amber-50 text-amber-700 text-[9px]">⏳ Pending</Badge>
                                     )}
                                  </div>

                                  <div className="flex items-center justify-between text-[11px] p-2 bg-muted/20 rounded-lg border">
                                     <span>Line 3: <strong>Department Head</strong> (Dr. James Okonkwo)</span>
                                     {signatures.deptHead.status === "complete" ? (
                                        <Badge className="bg-emerald-50 text-emerald-700 text-[9px]">✅ Signed {signatures.deptHead.date}</Badge>
                                     ) : (
                                        <Badge className="bg-slate-100 text-slate-400 text-[9px]">⏳ Blocked</Badge>
                                     )}
                                  </div>
                               </div>
                               ...
                               {currentStage === 3 && signatures.admin.status === "pending" && (
                                 <div className="mt-4 p-3 border rounded-xl bg-card space-y-3 shadow-inner animate-in fade-in">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                       ...
                                       <div className="space-y-1">
                                          <label className="text-[9px] font-bold text-muted-foreground uppercase">Handwritten Sig Box</label>
                                          <div className="flex items-center gap-2 h-8 px-2 border rounded-lg bg-card cursor-pointer" onClick={() => setAdminSigChecked(!adminSigChecked)}>
                                             <input 
                                                type="checkbox" 
                                                checked={adminSigChecked}
                                                onChange={(e) => setAdminSigChecked(e.target.checked)}
                                                className="w-4 h-4 rounded text-primary"
                                             />
                                             <span className="text-[10px] font-bold italic text-muted-foreground">Draw handwriting signature</span>
                                          </div>
                                       </div>
                                       <div className="space-y-1">
                                          <label className="text-[9px] font-bold text-muted-foreground uppercase">Date Signed</label>
                                          <input 
                                             type="text" 
                               ...
                               {currentStage === 3 && signatures.admin.status === "complete" && signatures.deptHead.status === "pending" && (
                                 <div className="mt-4 p-3 border rounded-xl bg-card space-y-3 shadow-inner animate-in fade-in">
                                    <p className="text-[10px] font-bold text-foreground uppercase tracking-wider flex items-center gap-1.5 text-primary"><FileSignature className="w-3.5 h-3.5" /> Sign as Department Head (Dr. James Okonkwo)</p>
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                       <div className="space-y-1">
                                          <label className="text-[9px] font-bold text-muted-foreground uppercase">Printed Name</label>
                                          <input 
                                             ...
                                             onChange={(e) => setDeptHeadNameInput(e.target.value)}
                                       ...
                                       <div className="space-y-1">
                                          <label className="text-[9px] font-bold text-muted-foreground uppercase">Handwritten Sig Box</label>
                                          <div className="flex items-center gap-2 h-8 px-2 border rounded-lg bg-card cursor-pointer" onClick={() => setDeptHeadSigChecked(!deptHeadSigChecked)}>
                                             <input 
                                                type="checkbox" 
                                                checked={deptHeadSigChecked}
                                                onChange={(e) => setDeptHeadSigChecked(e.target.checked)}
                                                className="w-4 h-4 rounded text-primary"
                                             />
                                             <span className="text-[10px] font-bold italic text-muted-foreground">Draw handwriting signature</span>
                                          </div>
                                       ...
                                       <div className="space-y-1">
                                          ...
                                          <input 
                                             ...
                                             onChange={(e) => setDeptHeadDateInput(e.target.value)}
                                             placeholder="MM/DD/YYYY"
                                             className="w-full text-xs h-8 px-2.5 border rounded-lg bg-card outline-none"
                                          />
                                       </div>
                               ...
                               {currentStage === 3 && signatures.admin.status === "complete" && signatures.deptHead.status === "complete" && (
                                  <div className="mt-4 p-3 bg-emerald-50 border border-emerald-100 dark:bg-emerald-950/20 dark:border-emerald-950 rounded-xl flex items-center justify-between">
                                     <span className="text-[10px] font-bold uppercase text-emerald-800">Checklist Status: Signatures Verified ✅</span>
                                     <Button 
                                       size="sm" 
                                       onClick={() => {
                                         ...
                                         setCurrentStage(4);
                                         setPayrollStatus('awaiting');
                                         logAction("STAGE3_COMPLETED", "System", "admin", "Signatures verified. Routed to Stage 4 Payroll.");
                                         showToast("Stage 3 Approved! Routed to Payroll.");
                                       }}
                         ...
                         {/* STAGE 4 */}
                         <div className="flex gap-6 relative z-10">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border ${currentStage > 4 ? "bg-emerald-500 text-white border-emerald-500" : currentStage === 4 ? "bg-white text-primary border-primary" : "bg-muted text-muted-foreground border-border"}`}>
                               {currentStage > 4 ? <CheckCircle2 className="w-4 h-4" /> : <span className="text-xs">4</span>}
                            </div>
                            <div className="flex-1">
                               <div className="flex items-center gap-2 mb-1 justify-between">
                                  <h3 className="text-xs font-bold text-foreground">Stage 4: Payroll Processing</h3>
                                  {getStageBadge(4, payrollStatus)}
                               </div>
                               <p className="text-[11px] text-muted-foreground leading-relaxed">
                                  Reimbursement is synced to Workday ERP and processed under IRS §127 tax split rules.
                               </p>
                               {currentStage === 4 && (
                                  <div className="mt-3 p-3 bg-muted/20 border rounded-xl flex items-center justify-between">
                                     <span className="text-[10px] font-bold uppercase text-muted-foreground">Status: Awaiting Post</span>
                                     <Button 
                                       size="sm" 
                                       onClick={() => {
                                         setPayrollStatus('processed');
                         ...
                         {/* STAGE 5 */}
                         <div className="flex gap-6 relative z-10">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border ${currentStage > 5 || paymentStatus === 'paid' ? "bg-emerald-500 text-white border-emerald-500" : currentStage === 5 ? "bg-white text-primary border-primary" : "bg-muted text-muted-foreground border-border"}`}>
                               {paymentStatus === 'paid' ? <CheckCircle2 className="w-4 h-4" /> : <span className="text-xs">5</span>}
                            </div>
                            <div className="flex-1">
                               <div className="flex items-center gap-2 mb-1 justify-between">
                                  <h3 className="text-xs font-bold text-foreground">Stage 5: Payment Completion</h3>
                                  {getStageBadge(5, paymentStatus)}
                               </div>
                               <p className="text-[11px] text-muted-foreground leading-relaxed">
                                  Payment posted to employee's bi-weekly paycheck distribution. 10-year immutable audit log finalized.
                               </p>
                               {currentStage === 5 && paymentStatus !== "paid" && (
                                  <div className="mt-3 p-3 bg-muted/20 border rounded-xl flex items-center justify-between">
                                     <span className="text-[10px] font-bold uppercase text-muted-foreground">Status: Scheduled</span>
                                     <Button 
                                       size="sm" 
                                       onClick={() => {
                                         setPaymentStatus('paid');
                                         logAction("STAGE5_COMPLETED", "System", "admin", "Reimbursement paid. Verification final.");
                                         showToast("Payment release verified! Case complete.");
                                       }}
                                       className="h-8 text-[10px] bg-emerald-600 hover:bg-emerald-700 font-bold text-white shadow-sm"
                                     >
                ...
                <TabsContent value="checklist" className="pt-0">
                   <Card className="border-border shadow-sm">
                      <CardHeader className="pb-2">
                         <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">SOP Section 10 checklist (14-Point Intake Review)</CardTitle>
                      </CardHeader>
                      <CardContent className="pt-4 px-6 pb-6 space-y-4">
                         <div className="p-3 bg-blue-50 border border-blue-200 dark:bg-blue-950/20 dark:border-blue-950 rounded-xl flex items-start gap-2.5 text-blue-800 dark:text-blue-300">
                            ...
                            <p className="text-[10px] leading-relaxed">
                               Verify application files against quality compliance checklist gates. Stage 1 routes automatically once all required items pass.
                            </p>
                         </div>
                                 ...
                                 <div key={item.id} className="py-3 flex items-center justify-between gap-4">
                                    <div className="min-w-0">
                                       <div className="flex items-center gap-2">
                                          ...
                                          {required ? (
                                             <Badge className="bg-rose-50 text-rose-700 text-[8px] font-bold h-4 px-1.5">Required</Badge>
                                          ) : (
                                             <Badge className="bg-slate-100 text-slate-400 text-[8px] font-bold h-4 px-1.5 border-transparent">N/A (NYSNA/Program)</Badge>
                                          )}
                                       </div>
                                       <p className="text-[10px] text-muted-foreground leading-tight mt-0.5">{item.desc}</p>
                                    </div>
                                    <div className="flex items-center gap-3">
                                       <span className={val ? "text-[10px] font-bold text-emerald-600" : "text-[10px] font-bold text-rose-600"}>
                                          {val ? "Pass" : "Fail"}
                                       </span>
                                       <Switch 
                                          checked={val} 
                ...
                <TabsContent value="audit" className="pt-0">
                   <Card className="border-border shadow-sm">
                      <CardHeader className="pb-2">
                         <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Immutable Compliance Audit History (10-Year Retention)</CardTitle>
                      </CardHeader>
                       <CardContent className="pt-4 px-0 pb-0">
                          <div className="overflow-x-auto">
                             ...
                             <TableBody>
                                {auditLogs.map((entry) => (
                                  <TableRow key={entry.id} className="border-b border-border/50 hover:bg-muted/20">
                                     ...
                                     <TableCell className="px-6 text-[10px] text-muted-foreground whitespace-nowrap">{entry.timestamp}</TableCell>
                                     <TableCell className="px-6 text-[10px] text-muted-foreground leading-relaxed">{entry.description}</TableCell>
                                  </TableRow>
                                ))}
                             </TableBody>
                             </Table>
        ...
        <aside className="w-full xl:w-80 bg-card border-t xl:border-t-0 xl:border-l border-border xl:overflow-y-auto flex-shrink-0 xl:h-full">
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 xl:grid-cols-1 gap-6">
