...
const RUBRIC_DESCRIPTIONS: Record<
  string,
  { range: string; label: string; desc: string }[]
> = {
  need: [
    {
      range: "1-2",
      label: "Low Need",
      desc: "Full tuition coverage already applies. No hardship stated or FAFSA EFC is high.",
    },
    {
      range: "3-4",
      label: "Moderate Need",
      desc: "Pays minor out-of-pocket costs. Standard curriculum fees apply.",
    },
    {
      range: "5-6",
      label: "Significant Need",
      desc: "Out-of-pocket expenses exceed 25% of monthly income. Standard limits don't cover full costs.",
    },
    {
      range: "7-8",
      label: "High Need",
      desc: "Substantial tuition balance remains. Multiple family dependencies or hardships documented.",
    },
    {
      range: "9-10",
      label: "Critical Need",
      desc: "Severe hardship. Candidate cannot enroll/continue without scholarship support.",
    },
  ],
  academic: [
    {
      ...
      desc: "GPA below 2.5 or program is not accredited.",
    },
    {
      range: "3-4",
      label: "Satisfactory",
      desc: "GPA 2.5 - 3.0. Fulfills standard course requirements.",
    },
    {
      range: "5-6",
      label: "Strong",
      desc: "GPA 3.0 - 3.4. Solid academic track record with positive reference reviews.",
    },
    {
      range: "7-8",
      label: "Superior",
      desc: "GPA 3.4 - 3.8. Consistent high performance in challenging courses.",
    },
    {
      range: "9-10",
      label: "Outstanding",
      desc: "GPA 3.8 - 4.0. Top academic honors, publishing or leadership in coursework.",
    },
  ],
  potential: [
    {
      range: "1-2",
      label: "Lacking",
      desc: "Recommendation letters are generic with no leadership endorsement.",
    },
    {
      range: "3-4",
      label: "Developing",
      desc: "Recommendations indicate potential to lead. Supervises minor shift tasks.",
    },
    {
      range: "5-6",
      label: "Competent",
      desc: "Confirms positive communication, team supervision, and coordination.",
    },
    {
      range: "7-8",
      label: "Distinguished",
      desc: "Highlights peer leadership, committee chairs, or clinical quality improvement.",
    },
    {
      range: "9-10",
      label: "Exceptional",
      desc: "Active union leadership/mentorship, pioneers team training, stellar testimonials.",
    },
  ],
};
...
export default function ScholarshipPortal() {
  ...
  useEffect(() => {
    const currentApplicantScore = mockCommitteeScores.find(
      (s) =>
        s.applicantId === selectedApplicantId && s.memberId === currentUser?.id,
    );
    setScores({
            ...
            {mockScholarshipApplicants.map((applicant) => (
              ...
              >
                <div className="flex justify-between items-start mb-1.5">
                  <span
                    className={`text-xs font-bold ${selectedApplicantId === applicant.id ? "text-primary" : "text-foreground"}`}
                  >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-1">
                    {applicant.conflictFlag && (
                      <Badge className="bg-amber-500/10 text-amber-600 border-amber-500/20 text-[8px] h-4">
                        Conflict
                      </Badge>
                    )}
                  </div>
                  <span className="text-[9px] font-bold text-muted-foreground uppercase">
                    {mockCommitteeScores.some(
          ...
          {selectedApplicant ? (
              ...
              <header className="bg-card border-b border-border p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <h1 className="text-xl font-bold font-display">
                    {selectedApplicant.anonymizedId}
                  </h1>
                  <p className="text-xs text-muted-foreground">
                    {selectedApplicant.degreeProgram} •{" "}
                  ...
                  <div className="xl:col-span-2 space-y-6">
                        ...
                        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              <Progress
                                value={92}
                                className="h-1 flex-1 bg-primary/10"
                              />
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              <Progress
                                value={100}
                                className="h-1 flex-1 bg-emerald-500/10"
                              />
                    ...
                    <Tabs defaultValue="rubric" className="w-full">
                      <TabsList className="bg-brand-lightblue p-1 h-auto rounded-xl inline-flex items-center gap-1 border-none mb-6">
                        <TabsTrigger
                          value="rubric"
                          className="rounded-lg px-4 py-2 text-xs font-bold data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all"
                        >
                          Scoring Rubric
                        </TabsTrigger>
                        <TabsTrigger
                          ...
                          {[
                            {
                              ...
                              desc: "Based on FAFSA/SAR and personal hardship statement.",
                              icon: Star,
                            },
                            {
                              id: "academic",
                              label: "Academic Standing",
                              desc: "Prior GPA and rigor of selected program.",
                              icon: Trophy,
                            },
                            {
                              id: "potential",
                              label: "Leadership Potential",
                              desc: "AI sentiment analysis of recommendation letters.",
                              icon: Zap,
                            },
                          ].map((criteria) => {
                            const currentScore = scores[criteria.id] ?? 5;
                            const selectedLevel = getRubricLevel(
                              criteria.id,
                              currentScore,
                            );
                            return (
                        ...
                        <Card className="border-border shadow-sm">
                          <CardContent className="p-8 text-center text-muted-foreground">
                            ...
                            <p className="text-xs mt-1">
                              This is a first-time scholarship applicant.
                            </p>
                          </CardContent>
                        </Card>
                      </TabsContent>
                    ...
                    <Card className="border-violet-500/20 bg-violet-500/5 shadow-sm overflow-hidden">
                      <CardHeader className="pb-3 border-b border-violet-500/10 bg-violet-500/5">
                        <div className="flex items-center gap-2">
                          ...
                          <CardTitle className="text-xs font-bold uppercase tracking-widest text-violet-700">
                            Decision Interface
                          </CardTitle>
                        </div>
                      </CardHeader>
                      <CardContent className="p-4 space-y-4">
                        <div className="p-3 bg-white rounded-lg border border-violet-200 space-y-2 shadow-sm">
                          <div className="flex items-center justify-between text-[10px] font-bold text-muted-foreground uppercase">
                            ...
                            <span>
                              {isSubmitted ? "2 of 5 Voted" : "1 of 5 Voted"}
                            </span>
                          </div>
                          <Progress
                            value={isSubmitted ? 40 : 20}
                            className="h-1 bg-violet-100"
                          />
                          <p className="text-[10px] text-muted-foreground leading-tight italic">
                            Final decision buttons will unlock once 5/5
                            committee members have submitted their private
                            scores.
                          </p>
                        </div>
                        ...
                        <div className="space-y-2 opacity-50 pointer-events-none">
                          ...
                          <Button
                            variant="outline"
                            className="w-full border-border text-foreground hover:bg-muted font-bold h-10"
                          >
                            Deny Application
                          </Button>
                        </div>
                      ...
                      <CardContent className="p-0">
                        {[
                          ...
                          >
                            ...
                            <Badge
                              variant="outline"
                              className={`text-[8px] h-4 px-1 ${reviewer.status === "completed" ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-muted text-muted-foreground border-transparent"}`}
                            >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
              ...
              <p className="text-sm mt-1">
                Select an anonymized profile from the left sidebar to begin
                review.
              </p>
            </div>
          )}
